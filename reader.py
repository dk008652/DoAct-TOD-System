...
class _ReaderBase(object):
    ...
    def _bucket_by_turn(self, encoded_data):
        #this recieves the data in encoded form
        turn_bucket = {}
        #dial_length = [] #new line added
        for dial in encoded_data:
            #print("_bucket_by_turn dialogue demo: ", dial) #new line added new if and else 
            
               
            turn_len = len(dial)
            #dial_length.append(len(dial)) #new line added
            if turn_len not in turn_bucket:
                turn_bucket[turn_len] = []
            
            turn_bucket[turn_len].append(dial)
            #print("turn_bucket:", turn_bucket) #new line added
            
        
        #print("size of turn_bucket:", len(turn_bucket)) #new line added
        #print("keys and values of turn bucket\n") #new line added
        #print("**reader.py/ _bucket_by_turn**") #new line
        """dialogue_, dialogue_length = [], [] #new line
        for key, value in sorted(turn_bucket.items()): #new line added/ new loop added
            dialogue_.append(key)
            dialogue_length.append(len(value))
            #print("type of value :", type(value))
       
        print("no of turn in the dialogue**:", dialogue_) #new lines added
        print("no of the dialogue of a particular turn_size**:", dialogue_length)"""
        
        #print("length of all dialogues:", dial_length) # new line added
        #print("no of dialogues in training dataset", len(dial_length)) #new line added
        
        del_l = []
        for k in turn_bucket:
            if k >= 5:
                del_l.append(k)
            #print("del_l:", del_l) #new line added
    ...
    def _construct_mini_batch(self, data):
        ...
        batch = []
        for dial in data:
            # if dial[0]['dial_id'][0] == 's' or dial[0]['dial_id'][0] == 'w':
            # if dial[0]['dial_id'][0] == 'p' or dial[0]['dial_id'][0] == 'm':
                # continue
            batch.append(dial)
    ...
    def inverse_transpose_turn_RL(self, turn_list):
        """
        eval, one dialog at a time
        """
        #print("reader/inverse_transpose_turn/****", turn_list) #new line added
        # dialogs = {}
        ...
        for turn_idx in range(turn_num):
            ...
            for key, value in turn.items():
                # print("key: ", key)  #new line
                # if key == 'turn_domain':
                #     print("key: ",turn['turn_domain'][-1])
                # if key=='dial_id':
                #     continue
                if key == 'pointer' and self.db is not None:
                    turn_domain = str(turn['turn_domain'][-1])
                    # print("type of turn_domain: ", type(turn_domain))
                    # print("2 turn_domain: ", (turn_domain))
                    value = self.db.pointerBack(value, turn_domain)
                    # print("value: ", value)
                dial_turn[key] = value
            dialogs.append(dial_turn)
        '''print("dialogue length", len(dialogs)) #new line added
        for key, value in dialogs.items(): #new line added/ loop
            print("key{}: value:{}".format(key, value))''' 
        
        # print("Reader/ Dialogue: ", dialogs)
        return dialogs
    ...
    def get_eval_data(self, set_name='dev'):
        ...
        num_turns = 0
        num_dials = len(dial)
        dial_4 = []  # dial with turn size of 4 or less
        dial_6 = []  # dial with turn size of 4 or less
        dial_8 = [] # dial with turn size of 8 or less
        dial_12 = [] # dial with turn size of 12 or less
        dial_20 = [] # dial with turn size of 20 or less
        for d in dial:
            num_turns += len(d)
            if len(d)<=4:
                dial_4.append(d)
            elif (len(d)>=5 and len(d)<=6):
                dial_6.append(d)
            elif (len(d)>=7 and len(d)<=8):
                dial_8.append(d)
            elif (len(d)>=9 and len(d)<=12):
                dial_12.append(d)
            elif (len(d)>=9 and len(d)<=20):
                dial_20.append(d)

        self.set_stats[set_name]['num_turns'] = num_turns
        self.set_stats[set_name]['num_dials'] = num_dials
        # print("no of dialogues in test set, dial_4 :", len(dial_4))
        # print("no of dialogues in test set, dial_6 :", len(dial_6))
        # print("no of dialogues in test set, dial_8 :", len(dial_8))
        print("no of dialogues in test set, dial_12 :", len(dial_12))
        # print("no of dialogues in test set, dial_20 :", len(dial_20))
        # print("Average turn size of dataset: ", num_turns/num_dials)
        # print("length of test dataset:", len(dial_4 + dial_8 + dial_12+ dial_20)/1000 )
        return dial_12
        
    def get_batches(self, set_name):
        # print("LOOK INTO THIS: reader.pyt/get_batches:") 
        # return self.train   # line modified to deal with llama for multigpu training
        """
        ...
        if cfg.low_resource and set_name == 'train':
            ...
            dial = random.sample(dial, int(len(dial)*cfg.percentage))  #line modified

            #print("type of dial:", type(dial)) #new line added
            #print("length of the dialogue:", len(dial)) #new line added
            logging.info('Low Resource setting, finetuning size: {}'.format(len(dial)))
        
        turn_bucket = self._bucket_by_turn(dial)
        '''print("type of turn bucket:", type(turn_bucket)) #new line added
        for key, value in turn_bucket.items(): #new line added new loop added
            print("key of turn_bucket:", key)
            print("length of value of turn_bucket:", len(value))
            
        
        print("******turn bucket details:******") #new line added
        print("size of turn bucket", len(turn_bucket)) #new line added'''
        ...
        #construct the minibatch of the dialogues which has turn_size=k #new line added
        print(" reader.py/_construct_mini_batch training only on multi domain datapoints")
        print("made changes into _construct_mini_batch to train in multi-domain datapoints")
        ...
        for k in turn_bucket:
            ...
            #print("******details of turn_size and no of dialogue of that turn_size**********") #new line added
            #print("value of K:", k) #new line added
            #print("size of a batch: ", len(batches)) #new line added            
            all_batches += batches
        
        #print("size of all_batches:", len(all_batches)) #new line added
...
class MultiWozReader(_ReaderBase):
    def __init__(self, tokenizer):
        ...
        #print("all_domain_list:", all_domains_list) #new line added
    ...
    def add_sepcial_tokens(self):
        ...
        for word in self.vocab._word2idx.keys():
            if word.startswith('[value_') and word.endswith(']'):
                special_tokens.append((word))   ## line modified
    ...
    def _build_vocab(self):
        ...
        #print("**cfg.vocab_path_eval**", cfg.vocab_path_eval) #new line added
        #print("**cfg.vocab_path_train**",cfg.vocab_path_train) #new line added
        #print("****reader.py/_build_vocab/Vocab path: ****", vp) #new line added
        # vp = cfg.vocab_path+'.json.freq.json'
        #print("reader/_build_vocab vocabulary path for training", vp)
    ...
    def _load_data(self, save_temp=True):
        ...
        if save_temp: # save encoded data
            ...
            if os.path.exists(encoded_file):
                ...
                #print("*********encoded data details****reader.py file************")
                #print("encoded type:", type(encoded_data)) #NEW LINE ADDED
                ...
                with open('test_data_processed.json', 'w') as json_file:
    ...
    def _get_encoded_data(self, fn, dial):
        ...
        for idx, t in enumerate(dial['log']):  # tokenize to list of ids
            ...
            #print("encoded data type:", type(encoded_dial)) #new line added
    ...
    def bspan_to_DBpointer(self, bspan, turn_domain):
        ...
        #print("****reader.py/bspan_to_DBpointer****") #new line added
        #print(constraint_dict) #new line added
    ...
    def convert_turn_eval(self, turn, pv_turn, first_turn=False):
        ...
        # predict_list = []
        prompt = ''
        #print("reader/convert_turn_eval: use_true_curr_bspn-->", cfg.use_true_curr_bspn) #new line added it is false
        if cfg.use_true_curr_bspn:
            #print("reader/convert_turn_eval: use_true_curr_aspn-->", cfg.use_true_curr_aspn) #new line added
    ...
    def convert_batch_session(self, dial_batch):
        ...
        concat [U_0, B_0, A_0, R_0, ... , U_n, B_n, A_n, R_n] n is the no of dialogues in the batch, n = batch size
        ...
        dialog_id = set()
        final_domain = [] # for RL
        final_domain_embed = [] # for domain embedding
        final_pointer = [] # for RL

        # print("******", dial_batch)
        cell_list = ['user', 'bspn', 'db','aspn', 'resp']  # line modified  for generating non-delexicaized response, resp_non_delex
        for idx, dial in enumerate(dial_batch):
            ...
            domain_set = set()
            domain_turn = [] # for RL
            domain_turn_embed = [] # for domain embedding
            ...
            for turn_num, turn in enumerate(dial):
                ...
                for cell in cell_list:
                    ...
                    #print("turn['cell']:", turn[cell]) #new line added
                
                if not (turn['turn_domain'][0].strip('[]') == 'general'):  # new lines
                    # x = self.tokenizer.encode(turn['turn_domain'][0].strip('[]'))  # with encoding
                    x = turn['turn_domain'][0].strip('[]') # without encoding
                    # domain_set.add(x[0]) #new line, with encoding
                    domain_set.add(x) #new line
                    domain_turn_embed.append(x) #new line
                    # print(self.tokenizer.decode(x))
                dialog_id.add(turn['dial_id']) # for RL
                domain_turn.append(turn['turn_domain'][0]) # for RL
                # print("domain_tun ", domain_turn) 
                pointer_turn.append(turn['pointer'])

            contexts.append(context)
            domains.append((list(domain_set)))  # for domain classification
            # domains.append(self.tokenizer(list(domain_set))['input_ids'])  # for MOE
            final_domain.append(domain_turn) # for RL
            final_domain_embed.append(domain_turn_embed) # for RL
            final_pointer.append(pointer_turn) # for RL

        inputs['contexts'] = contexts

        if len(inputs['contexts'])==0:  
            return None
            # print("reader.py/ input length is zero")
        inputs['contexts_np'], inputs['lengths'] = utils.padSeqs_gpt(inputs['contexts'], cfg.pad_id)
        # print("domain: ", domains)
        inputs['contexts_domain'] = (domains)  ## for domain classification // DCTOD, uncomment this and comment the line below to use it for domain classification
        # print("inputs['contexts_domain']", inputs['contexts_domain'])
        inputs['dial_id'] = list(dialog_id) #RL
        inputs['domain_turn'] = final_domain #RL
        # print("final_domain_embed: ", final_domain_embed)
        inputs['domain_turn_embed'] = final_domain_embed # for adding domain embedding
        inputs['pointer'] = final_pointer #RL

        ### MOE
        # domain = []
        # for i in range(len(inputs['domain_turn'])):
        #     domain_tokenized = self.tokenizer(inputs['contexts_domain'][i])['input_ids']
        #     domain_tokenized = [token for sublist in domain_tokenized for token in sublist]
        #     domain.append(domain_tokenized)
        # # print("domain_tokenzied", domain)
        # inputs['contexts_domain'] = domain
        # # inputs['contexts_domain'], inputs['domain_lengths'] = utils.padSeqs_gpt(inputs['contexts_domain'], cfg.pad_id)
        # # print("inputs['contexts_domain']: ", inputs['contexts_domain'])
    ...
    '''def convert_batch_session(self, dial_batch):
        ...
        for idx, dial in enumerate(dial_batch):
            ...
            for turn_num, turn in enumerate(dial):
                ...
                #print("reader/convert_batch_session/turn.items():",turn.items()) #new line added
                for cell in cell_list:
                    context.extend(turn[cell])
                    #print("turn['cell']:", turn[cell]) #new line added
            ...
            #print("context of one dialogue:",context) #new line added
            contexts.append(context)
            domains.append(list(domain_set))
        
        #print("contexts of one batch:", contexts) #new line added
        inputs['contexts'] = contexts
        
        inputs['contexts_np'], inputs['lengths'] = utils.padSeqs_gpt(inputs['contexts'], cfg.pad_id)
        inputs['contexts_domain'] = (domains)
    ...
    def convert_batch_turn(self, turn_batch, pv_batch, first_turn=False):
        ...
        if first_turn:
            ...
            # print("turn_batch['turn_domain'][0][0]", type((turn_batch['turn_domain'][0][0]).strip('[]')))
            # print("inputs['contexts_np'].shape",inputs['contexts_np'].shape)
            # print("turn_batch['turn_domain']", turn_batch['turn_domain'] )
            
            inputs['contexts_domain'] = turn_batch['turn_domain']   #(turn_batch['turn_domain'][0][0]).strip('[]') # new line turn_batch['turn_domain']:  [['[restaurant]'], ['[restaurant]']]
            ...
            inputs['contexts_domain'] = turn_batch['turn_domain']  #new line
